...
심박수, 체온, 서브모터 반응을 포함한 자율 기능 등을 포함한 요소들은 혈당에 직접적인 영향을 미치지 않을 수 있습니다.  
**즉, 혈당에 영향을 미치고 관련된 요인은 극히 개인화된 것입니다.**  
...
개인화된 포도당 excursion classes(그림 2 원)는 포도당 변동에 대한 더 많은 맞춤형 정보를 제공하고 혈당에 영향을 미치는 식단, 운동 및 스트레스 관련 행동의 자기 관리 및 추적을 가능하게 합니다.  
**또한 혈당 기준선은 시간이 지남에 따라 동적이므로 포도당 excursion을 정의하기 위한 단일 값 모집단 수준 임계값은 개인의 일반적인 상태로부터의 시간에 따른 편차를 이해하기에 부적절**합니다.  
따라서 **개인화된 포도당 배설의 유무를 나타내기 위해 각 간질 포도당 측정을 분류하기 위한 세 가지 개인화된 동적 지정을 개발**했습니다.  

**이러한 범주를 각각 24시간 개인화된 평균보다 크거나 작거나 하나의 표준 편차 내에 있는 간질 포도당 측정에 해당하는 PersHigh, PersLow 및 PersNorm으로 표시합니다**(그림 3a).  
...
이것이 가능한지 여부를 결정하기 위해, 간질 포도당이 개인 표준을 벗어난 때를 감지하는 것을 목표로 이러한 대체 측정 모드의 데이터를 사용하여 분류 모델을 개발했습니다.  
  
#### 3.2.2. 69개의 변수 엔지니어링
데이터 기반 및 도메인 기반 기능 엔지니어링의 조합을 사용하여 **예측 모델에 입력으로 이전에 설명한 문헌을 기반으로 69개의 변수를 엔지니어링**했습니다([보충 표 1](https://static-content.springer.com/esm/art%3A10.1038%2Fs41746-021-00465-w/MediaObjects/41746_2021_465_MOESM1_ESM.pdf)).  
...
**CGM 측정의 개인화된 롤링 기반을 기반으로 PersHigh, PersLow 또는 PersNorm으로 분류된 개인 내 excursion의 기본 진위를 정의**했습니다.  
...
간질 포도당 예측에 대한 69개 변수 각각의 기여도를 결정하기 위해, LOPOCV Random forest regression 모델의 각 변수에 대한 불순도 기반 중요도를 각 폴드에 걸쳐 평균화된 중요도로 계산했습니다.  
...
Feature들은 Empatica E4 손목 착용 웨어러블(wearable)과 음식 일기에서 수집한 데이터를 기반으로 설계되었습니다.  
식이요법, 스트레스, 운동, 일주기 리듬 및 행동 습관과 관련된 기능(혈당 변동에 기여하는 것으로 알려진 모든 기능)을 포함하여 총 69개의 데이터 기반, 도메인 기반 및 인구 통계학적 과거 기능이 모델 개발에 사용되었습니다.  
이 Feature들은 (적절한 경우, 아래에 설명된 바와 같이) 롤링 기준으로 5분마다 계산되었습니다.  
모델링에 사용된 모든 특징들은 과거형(측정이 예측되기 5분에서 24시간 전)이었습니다. 따라서, 모델들은 예측을 할 수 있는 과거형 데이터만 제공받았습니다.  

69개의 특징 중 두 가지는 사용자 입력을 모델에 통합한 인구통계학적 데이터(demographic data)였습니다: 생물학적 성별과 HbA1c. 또 다른 특징인 '개인화' 기능은 모델들이 개인적일 수 있는 관계를 배울 수 있도록 각 참가자를 고유한 숫자로 구분합니다.  

**스마트워치 데이터의 5분 간격별 데이터 기반 특징에는 센서별로 7개의 요약 통계(mean, standard deviation, minimum, maximum, first quartile, third quartile, skew)가 포함됩니다.**  
**이 요약 통계는 심박수, 가속도 측정(3축의 벡터 크기), 전극 활동 및 피부 온도에 대해 계산됩니다.**  

Domain 기반 기능 엔지니어링의 경우 혈당에 미치는 영향을 입증한 스트레스, 운동(단기 및 장기 효과), 생체 리듬 및 식이요법(식사 타이밍, 식사 빈도, 단백질, 설탕, 탄수화물 및 칼로리의 단기, 중기 및 장기 효과 포함)의 4가지 요인에 초점을 맞추었습니다.  
이 각 요소를 세분화하고 아래 계산된 메트릭을 설명합니다:

- Electrodermal activity data(Stress) metrics  
**스트레스의 영향**은 심리학 연구에서 수십 년 동안 생리학적으로 측정되어 왔으며 여기서는 스트레스를 정량화하기 위해 그들의 몇 가지 방법을 차용합니다.  
전극 활동 데이터를 사용하여 피크를 감지하고 파이썬의 SciPy 라이브러리를 사용하여 피크의 prominences를 결정했습니다.  
**'unique peak'로 간주되려면 1초(4개의 데이터 포인트)의 피크 사이의 거리와 0.3 micro-simens의 prominence가 요구됩니다.**  
각 5분 간격으로 피크 수를 결정했습니다.  
그런 다음 **rolling window 접근 방식을 사용하여 이 데이터를 집계**했습니다.  
지난 2시간 동안의 rolling window에서 각 5분 간격으로 총 피크 수와 평균 피크 수를 결정했습니다.  

- HRV(Heart rate variability) metrics  
**심박변이도(HRV)는 급성 및 만성 스트레스와 관련하여 변동**하는 것으로 입증되었습니다.  
PPG에서 파생된 비트 간 데이터를 활용하여 각 5분 간격에 걸쳐 8개의 HRV 메트릭을 계산했습니다.  
계산된 메트릭에는 평균 HRV, 중앙값 HRV, 최대 HRV, 최소 HRV, 간격 표준 편차(standard deviation of intervals, SDNN), 간격의 연속 차이의 평균 제곱근(root mean square of successive differences in the intervals, RMSSD), 50ms 이상 차이가 나는 연속 간격 수(NN50), NN50의 비율을 전체 간격 수로 나눈 값(pNN50)이 포함됩니다.  

- Activity bout metrics  
**운동**이 혈당 수치에 미치는 단기 및 장기적 영향이 모두 있는 것으로 나타났으며, **가속도 측정 및 심박수 데이터를 모두 사용하여 조사하고자 했습니다.**  
사용자의 입력 없이 "운동"을 정량화하는 것은 어렵지만, **참가자들이 언제 더 활동적인지 결정**할 수 있습니다. 그래서 "activity bout(활동 기간)" 또는 활동에 대한 계산을 개발했습니다.  
"activity bout"을 계산하기 위해 5분 간격 동안 평균 가속도 벡터 magnitude와 평균 심박수를 측정하여 개인의 이전 과거 데이터 평균과 비교했습니다.  
5분 간격에 대한 평균 가속도 측정 및 평균 심박수 값이 모두 이전 평균보다 높으면 해당 간격은 "activity bout"이라고 합니다.  
포도당 대사에 대한 활동(activity)의 장단기 효과를 설명하기 위해 rolling window를 사용하여 마지막 시간의 총 activity bouts와 이전 24시간의 평균 activity bouts를 계산했습니다.  
또한 rolling window 접근법을 사용하여 지난 두 시간 동안의 평균 및 최대 가속도 벡터 magnitude를 찾았습니다.  

- Circadian rhythm metrics  
**일주기 리듬(Circadian rhythm)은** 대부분의 생리학에서 교란 변수이며 특히 일주기 리듬과 혈당 사이에 알려진 연관성이 있기 때문에 이를 특징으로 고려하는 것이 중요합니다.  
'자정부터의 분'과 '자정부터의 시간'을 일주기 리듬을 나타내는 특징으로 계산했습니다.  
개인이 깨어나는 시간은 일주기 리듬에 영향을 미칠 수 있습니다.  
**심박수 및 가속도는 깨어있는 상태와 수면 상태를 결정하는 데 사용**됩니다.  
각 참가자의 **각 하루에 대해, 가속도 및 심박수 평균과 표준 편차가 그날의 평균보다 작을 때를 조사**했습니다.  
**네 가지 측정값 중 두 가지 측정값이 하루의 평균보다 작을 때, 그 구간에 '0'을 할당**했습니다. **다른 모든 구간에는 '1'이 할당**되었습니다.  
그런 다음 rolling window 접근법을 사용하여 이 데이터를 3시간 동안을 평균화했습니다.  
이 데이터의 기울기가 급격히 변화하고 wake time 후 25분과 75분 후에 지속적으로 더 높게 유지된 경우에 'Wake Time(깨는 시간)'을 할당했습니다.  

- Diet metrics  
**식단**은 포도당 대사에 강력한 영향을 미치는 것으로 알려져 있습니다. 여기서 **음식이 포도당에 미치는 단기, 중기, 장기적인 영향을 설명하는 특징을 결정**했습니다. rolling window를 사용하여 2시간, 8시간, 24시간의 세 가지 윈도우에 걸쳐 칼로리, 단백질 그램, 탄수화물 그램, 설탕 그램을 합했습니다.  
참가자가 독특한 식사, 간식 또는 칼로리 음료를 마실 때마다, 그 간격을 이진법 '1'로 할당했습니다. 이것은 식사의 타이밍과 그것이 일주기 리듬 특징과 함께 포도당 대사에 어떻게 영향을 미칠 수 있는지를 결정할 수 있게 해줍니다.  
그런 다음 한 개인이 2시간, 8시간, 24시간에 걸쳐 얼마나 많이 소비했는지에 대한 rolling sum을 취했습니다.  
또한 한 개인이 2시간, 8시간, 24시간 롤링 윈도우에 걸쳐 평균적으로 소비한 횟수를 조사했습니다.  